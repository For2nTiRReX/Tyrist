...
var HotelService = (function () {
    function HotelService(http) {
        this.http = http;
        this.url = "http://tyrist.dev/api/HotelsController.php";
        ...
        // console.log(this.serviceSubscriber);
    ...
    HotelService.prototype.extractHotels = function (response) {
        ...
        for (var i = 0; i < res.length; i++) {
            Hotels.push(new Hotel_1.Hotel(res[i].id, res[i].title, res[i].content, res[i].price, res[i].available, res[i].whereabout, res[i].country, res[i].thumbnail, res[i].comfort));
        }
        return Hotels;
    };
